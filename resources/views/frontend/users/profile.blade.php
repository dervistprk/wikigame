    ...
    <div class="container">
        ...
        <div class="game-info m-3 p-3 rounded">
            @if(\Auth::user()->isAdmin())
                ...
                <a class="btn btn-warning mb-2 @if($ip_check_message) disabled @endif" target="_blank"
                   href="{{ route('admin.dashboard') }}"><i class="fa fa-user-cog"></i> Yönetici Paneli</a>
                @if($ip_check_message)
                    <span class="text-danger">{{ $ip_check_message }}</span>
                @endif
            @endif
            <p>
                Profilim sayfasına hoşgeldiniz. Buradan profilinizle ilgili çeşitli işlemler gerçekleştirebilir,
                bilgilerinize göz atabilirsiniz. Alttaki butonlardan ilgili işlemleri gerçekleştirebilirsiniz.
            </p>
        </div>
