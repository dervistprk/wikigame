    ...
    <div class="mt-4">
        ...
        @if(Auth::check())
            @if(Auth::user()->id == $user->id || Auth::user()->isAdmin())
                ...
                <a href="#!" class="link-muted comment-action-links delete-comment-button" data-id="{{ $comment->id }}" data-toggle="tooltip" data-bs-placement="top" title="Yorumu Sil"><i class="fas fa-trash-alt ms-2"></i></a>
            @endif
            @if(Auth::user()->id != $user->id)
                ...
                <a href="#!" class="link-muted comment-action-links like-comment-button" data-id="{{ $comment->id }}" data-toggle="tooltip" data-bs-placement="top" title="Yorumu Beğen" @if($disabled_like_button) style="pointer-events: none" @endif><i class="fas fa-thumbs-up ms-2"></i></a>
                @if($comment->likes > 0)
                    <span class="badge bg-primary rounded-pill like-count">{{ $comment->likes }}</span>
                @endif
                <a href="#!" class="link-muted comment-action-links dislike-comment-button" data-id="{{ $comment->id }}" data-toggle="tooltip" data-bs-placement="top" title="Yorumu Beğenme" @if($disabled_dislike_button) style="pointer-events: none" @endif><i class="fas fa-thumbs-down ms-2"></i></a>
                @if($comment->dislikes > 0)
                    <span class="badge bg-danger rounded-pill dislike-count">{{ $comment->dislikes }}</span>
                @endif
            @else
                <a href="#!" class="link-muted comment-action-links like-comment-button" data-id="{{ $comment->id }}" data-toggle="tooltip" data-bs-placement="top" title="Yorumu Beğen" style="pointer-events: none"><i class="fas fa-thumbs-up ms-2"></i></a>
                @if($comment->likes > 0)
                    <span class="badge bg-primary rounded-pill like-count">{{ $comment->likes }}</span>
                @endif
                <a href="#!" class="link-muted comment-action-links dislike-comment-button" data-id="{{ $comment->id }}" data-toggle="tooltip" data-bs-placement="top" title="Yorumu Beğenme" style="pointer-events: none"><i class="fas fa-thumbs-down ms-2"></i></a>
                @if($comment->dislikes > 0)
                    <span class="badge bg-danger rounded-pill dislike-count">{{ $comment->dislikes }}</span>
                @endif
            @endif
        ...
        <div class="d-flex align-items-center mb-3">
            <p class="mb-0" data-toggle="tooltip" data-bs-placement="top" title="{{ Carbon\Carbon::parse($comment->created_at)->format('d/m/Y H:i:s') }}">
